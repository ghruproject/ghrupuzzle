...
export const outbreakPracticeDefinition: ExerciseDefinition<ShortReadSample> = {
  ...
  subtitle: 'Build a mapping-based phylogeny and submit sample-level QC, species, and outbreak-cluster calls.',
  summary:
  ...
  instructions: [
    ...
    'Create and review a phylogenetic tree in Newick format with every suitable sample represented.',
    'Identify likely outbreak clusters and submit the CSV or TSV result sheet for automated assessment.',
  ],
  sampleSheetIntro: 'Record the QC, species, and analytical cluster interpretation for every sample.',
  submissionText: 'Use the tree to support your cluster assignments, but submit only the completed result sheet. Do not upload the tree or a separate metadata file.',
  answerColumns: [
    { name: 'sample_id', description: 'Public sample identifier.' },
    { name: 'qc_status', description: 'PASS when the sample is suitable for analysis; otherwise FAIL.' },
    { name: 'failure_reason', description: 'NONE for a passing sample; otherwise one categorical failure reason.' },
    { name: 'species', description: 'Canonical organism call.' },
    { name: 'cluster', description: 'Your inferred outbreak grouping for a passing sample.' },
    { name: 'notes', description: 'Optional concise interpretation.' },
  ],
  sampleColumns: [
    { key: 'public_name', label: 'Sample' },
    { key: 'R1_URL', label: 'R1 FASTQ', isFile: true },
